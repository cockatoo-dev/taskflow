...
export default defineEventHandler(async (e) => {
  try {
    ...
    if (!canEdit(boardInfo.isOwner, boardInfo.publicPerms)) {
      throw forbiddenError("You do not have permission to add tasks to this board.")
    }
    const boardTasks = await db.getBoardTasksInfo(bodyData.boardId)
    if (boardTasks.length >= 80) {
      throw badRequestError("You've reached the maximum number of tasks for this board. Consider deleting an existing task before creating a new task.")
    }
